...
function Chart(props) {
  ...
  React.useEffect(function () {
    ...
    var xGroup = d3.scaleBand().range([0, 2 * Math.PI]).align(0);
    ...
    y.domain([0, d3.max(data, function (_ref) {
      var total = _ref.total;
      return total;
    }) > dataMax ? d3.max(data, function (_ref2) {
      var total = _ref2.total;
      return total;
    }) : dataMax]);
    angle.domain([0, d3.max(data, function (_, i) {
      return i + 1;
    })]);
    radius.domain([0, d3.max(data, function () {
      return 0;
    })]);
    var angleOffset = -360.0 / data.length / 2.0;
    var stackGen = d3.stack().keys(dataKeys);
    var arcVal = d3.arc().innerRadius(function (d) {
      return Number(y(d[0]));
    ...
    var arc = arcParent.selectAll("path").data(function (d) {
      return d;
    }).enter().append("path");
    arc // @ts-ignore
    ...
    }).on("mouseover", lodash.debounce(function (_event, d) {
    ...
    }, 0)).on("mouseout", lodash.debounce(function (_event, d) {
      var classname = this.getAttribute("class") || "arc_1";
      var i = classname.split("_")[1];
      this.setAttribute("fill", String(d.data.color) || "#ffffff"); // this.setAttribute("style", "opacity: 0");

      d3.selectAll(".tooltip_".concat(i)).remove();
    }, 0));
  ...
  return /*#__PURE__*/React.createElement(AxisContainer, {
...
var AxisContainer = styled__default['default'].div(_templateObject());
var Axis = styled__default['default'].svg(_templateObject2());
